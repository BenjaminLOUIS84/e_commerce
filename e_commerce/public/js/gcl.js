...
$(document).ready(function() { // Une fois que le document (base.html.twig) HTML/CSS a bien été complètement chargé...
    ...
    // add-collection-widget.js : fonction permettant d'ajouter un nouveau bloc "commandeLivre" au sein d'une commande (pour agrandir la collection)
    $('.add-another-collection-widget').click(function (e) {
        var list = $($(this).attr('data-list-selector'))
        
        // Récupération du nombre actuel d'élément "commandeLivre" dans la collection (à défaut, utilisation de la longueur de la collection)
        var counter = list.data('widget-counter') || list.children().length
        
        // Récupération de l'identifiant du commande concernée, en cours de création/modification
        var commande = list.data('commande')
        ...
        // par un numéro unique au sein de la collection de "commandeLivres" : ce numéro sera la valeur du compteur
        // courant (équivalent à l'index du prochain champ, en cours d'ajout).
        // Au final, l'attribut ressemblera à "commande[commandeLivres][n°]"
        newWidget = newWidget.replace(/__name__/g, counter)
        // Ajout également des attributs personnalisés "class" et "value", qui n'apparaissent pas dans le prototype original 
        newWidget = newWidget.replace(/><input type="hidden"/, ' class="borders"><input type="hidden" value="'+commande+'"')
    ...
    // anonymize-collection-widget.js : fonction permettant de supprimer un bloc "commandeLivre" existant au sein d'une commande
    $('.remove-collection-widget').find('div.borders').each(function() {
        addDeleteLink($(this))
    })
    // fonction permettant l'ajout d'un bouton "Supprimer ce livre" dans un bloc "commandeLivre", et d'enregistrer l'évenement "click" associé
    function addDeleteLink($livreForm) {
        var $removeFormButton = $('<div class="block"><button type="button" class="button">Supprimer ce livre</button></div>');
        $livreForm.append($removeFormButton)
    
        $removeFormButton.on('click', function(e) {
            $livreForm.remove()
        })
    }
    // remove-commande.js : fonction permettant de demander la confirmation de suppression d'une commande
    $('.remove-commande-confirm').on('click', function(e) {
        e.preventDefault()
        let id=$(this).data('id')
        let href=$(this).attr('href')
        showModalConfirm(id, href, "Confirmation de suppression d'une commande")
    })
